#tableContinuous2 is a slight alteration of tableContinuous (from the reporttools package). It adds the argument float (default TRUE).
#and changes line 197 (float<-TRUE becomes float<-float). As a result, setting longtable=FALSE and float=FALSE results in a sparse latex table.
...
tableContinuous2<-function (vars, weights = NA, subset = NA, group = NA, stats = c("n", 
                                                                 ...
                                                                 "max", "s", "iqr", "na"), prec = 1, 
          col.tit = NA, col.tit.font = c("bf", "", "sf", 
                                         "it", "rm"), print.pval = c("none", 
                                                                     "anova", "kruskal"), pval.bound = 10^-4, 
          declare.zero = 10^-10, cap = "", lab = "", font.size = "footnotesize", 
          longtable = TRUE, disp.cols = NA, nams = NA, float=TRUE,...) 
{
  col.tit0 <- col.tit
  ...
  if (identical(col.tit, NA) == TRUE) {
    ...
    col.tit <- c(fonts$text("Variable"), fonts$text("Levels"), 
                 ...
                 fonts$math("q_3"), fonts$text("Max"), 
                 fonts$math("s"), fonts$text("IQR"), fonts$text("\\#NA"))
  }
  ...
  for (i in 1:n.var) {
    ...
    if (identical(group, NA) == FALSE) {
      ...
      splitweights<-split(weights2, group)
      }
    for (j in 1:n.levels) {
      tmp <- as.vector(splits[[j]])
      tmpw<- as.vector(splitweights[[j]])
      if (sum(is.na(tmp) == FALSE) != 0) {
        ...
        out[ind[j], 7] <- WeightedMean(tmp,w=tmpw, na.rm = TRUE)
        out[ind[j], 8] <- quantile(tmp, 0.75, na.rm = TRUE)
        out[ind[j], 9] <- max(tmp, na.rm = TRUE)
        out[ind[j], 10] <- WeightedSE(tmp,w=tmpw, na.rm = TRUE)
    ...
    out[max(ind), 7] <- WeightedMean(vi,w=weights2, na.rm = TRUE)
    out[max(ind), 8] <- quantile(vi, 0.75, na.rm = TRUE)
    out[max(ind), 9] <- max(vi, na.rm = TRUE)
    out[max(ind), 10] <- WeightedSE(vi,w=weights2, na.rm = TRUE)
