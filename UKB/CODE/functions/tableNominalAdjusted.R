...
#+ When showgrouptotal = FALSE, the total observations of the given subgroups will not be reported
## Furthermore, the argument floater is added, with default value TRUE. When longtable is set to FALSE and 
#+ floater is set to false, the table will be a sparse tabular environment (in latex),r ather than a float environment
#+ (\beggin{table}...\end{table}.
##reporttools must be loaded for this function to work.

tableNominalAdjusted <- function (vars, weights = NA, subset = NA, group = NA, miss.cat = NA, 
          print.pval = c("none", "fisher", "chi2"), 
          pval.bound = 10^-4, fisher.B = 2000, vertical = TRUE, cap = "", 
          lab = "", col.tit.font = c("bf", "", "sf", 
                                     "it", "rm"), font.size = "footnotesize", 
          longtable = TRUE, nams = NA, cumsum = TRUE, showgrouptotal=TRUE,floater=TRUE,...) 
{
  ...
  showgrouptotal <- as.logical(showgrouptotal)
  stopifnot(identical(length(cumsum), 1L))
  stopifnot(identical(length(showgrouptotal), 1L))
  nColPerGroup <- 2L + as.integer(cumsum)
  out <- matrix(NA, ncol = 2 + nColPerGroup * (n.group + as.integer(showgrouptotal)), 
  ...
  for (i in 1:n.var) {
    ...
    indNA <- (is.na(g1) == FALSE) & (g1 != "NA") & 
      (is.na(v1) == FALSE) & (v1 != "NA")
  ...
  if (n.group > 1) {
    if (showgrouptotal) {    dimnames(out)[[2]] <- c(fonts$text("Variable"), 
                                                     fonts$text("Levels"), fonts$math(paste(col.tit, 
                                                                                            "_{\\mathrm{", rep(c(levels(group), "all"), 
                                                                                                               each = nColPerGroup), "}}", sep = "")))
    }else{  dimnames(out)[[2]] <- c(fonts$text("Variable"), 
                                  fonts$text("Levels"), fonts$math(paste(col.tit, 
                                                                         "_{\\mathrm{", rep(c(levels(group)), 
                                                                                            each = nColPerGroup), "}}", sep = "")))}
    for (i in 1:(n.group-1+as.integer(showgrouptotal))) {
      al <- paste(al, vert.lin, groupAlign, sep = "")
    }
    xtab1 <- xtable::xtable(out, digits = c(rep(0, 3), rep(digits, 
                                                           n.group + as.integer(showgrouptotal))), align = al, caption = cap, label = lab)
  ...
  if (n.group == 1) {
    ...
    dimnames(out)[[2]] <- c(fonts$text("Variable"), 
                            fonts$text("Levels"), fonts$math(col.tit))
